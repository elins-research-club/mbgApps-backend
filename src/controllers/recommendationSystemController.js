...
function buildModel(foods, current, goal) {
  ...
  for (const [nutrient, goalVal] of Object.entries(goal)) {
    ...
    if (diff > 0) {
      ...
      model.constraints[nutrient] = {
        min: 0,
        max: target[nutrient] * 1.05,
      };
    }
  }
  for (const foodName of Object.keys(foods)) {
    model.constraints[foodName] = { min: 0, max: 5 };
  }
  return model;
}
...
function getRecommendation(
  ...
  if (
    ...
    Object.keys(currentFoods).length === 0
  ) {
    console.warn("⚠️ getRecommendation() dipanggil tapi foods kosong");
    return { saran: [], kekurangan: [] };
  }

  if (!currentNutrition || Object.keys(currentNutrition).length === 0) {
    console.warn("⚠️ getRecommendation() dipanggil tapi nutrition kosong");
    return { saran: [], kekurangan: [] };
  }

  if (!goals[classGrade]) {
    console.warn("⚠️ getRecommendation() classGrade tidak valid:", classGrade);
    classGrade = 1;
  }
  ...
  const saran = Object.entries(servingsToAdd)
    ...
    .map(([nama, jumlahPorsi]) => ({
      ...
      porsi: parseFloat(jumlahPorsi.toFixed(2)),
    }));
