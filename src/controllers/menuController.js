  ...
  try {
    ...
    for (const ingredient of suggested_ingredients) {
      ...
      if (searchResult.bahan) {
        ...
        console.log(
          `⚠️ "${ingredient.nama}" → TIDAK DITEMUKAN di database, perlu alternatif.`
        );
      ...
      if (bahanIdToUse !== null) {
        if (!processedBahanIds.has(bahanIdToUse)) {
          ...
          console.log(
            `⚠️ DUPLIKAT Terdeteksi untuk bahan ID ${bahanIdToUse} ("${searchResult.bahan.nama}"), dilewati.`
          );
        }
      }
    }
        ...
        if (altResult.alternative_found && altResult.alternative_name) {
          ...
          if (altBahan) {
            ...
            if (!processedBahanIds.has(altBahan.id)) {
              ...
              console.log(
                `⚠️ DUPLIKAT (via Alternatif) Terdeteksi untuk bahan ID ${altBahan.id} ("${altBahan.nama}"), dilewati.`
              );
            }
            foundViaAlternative = true;
          } else {
            console.log(
              `⚠️ Alternatif "${altResult.alternative_name}" (disarankan AI) tidak ditemukan di DB`
            );
          }
        } else {
          console.log(
            `⚠️ Tidak ada alternatif scientific untuk "${missing.nama}"`
          );
        ...
        if (!foundViaAlternative) {
          ...
          if (generatedNutrition) {
            ...
            if (!processedBahanIds.has(newOrUpdatedBahan.id)) {
              ...
              console.log(
                `⚠️ DUPLIKAT (via AI Generate) Terdeteksi untuk bahan ID ${newOrUpdatedBahan.id} ("${newOrUpdatedBahan.nama}"), dilewati.`
              );
            }
          } else {
            console.log(
