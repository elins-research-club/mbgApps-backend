...
// ✅ FUNGSI BARU: Update kategori menu komposisi
async function updateCompositionCategory() {
  console.log("\n🔄 Tahap 4: Mengupdate kategori menu komposisi...");

  // Cek apakah ada menu dengan kategori "Komposisi Chef"
  const menusToUpdate = await prisma.menu.findMany({
    where: { kategori: "Komposisi Chef" },
  });

  if (menusToUpdate.length > 0) {
    console.log(
      `📊 Ditemukan ${menusToUpdate.length} menu dengan kategori "Komposisi Chef"`
    );

    // Update ke "komposisi_chef"
    const result = await prisma.menu.updateMany({
      where: { kategori: "Komposisi Chef" },
      data: { kategori: "komposisi_chef" },
    });

    console.log(
      `✅ Berhasil mengupdate ${result.count} menu dari "Komposisi Chef" → "komposisi_chef"`
    );
  } else {
    console.log(
      'ℹ️  Tidak ada menu dengan kategori "Komposisi Chef" yang perlu diupdate.'
    );
  }
}
...
async function main() {
  ...
  await prisma.menuKomposisi.deleteMany({}); // ✅ TAMBAHAN: Hapus juga relasi komposisi
  ...
  // ✅ TAMBAHAN: Update kategori menu komposisi
  await updateCompositionCategory();
